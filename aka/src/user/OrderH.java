...
/**
 * Servlet implementation class OrderH
 */
@WebServlet("/OrderH")
		...
		//   String user_id = null;
			...
			try 
			{
				...
				for(Cookie cookie : cookies)
				{
					...
					System.out.println("Entered book history section");
					String getall = "select * from `corder` where cid=" + user_id;
					System.out.println(getall);
					RequestDispatcher view=request.getRequestDispatcher("orderh.jsp");
			       ...
			       // System.out.print(count);
							 ...
							 while(myRs3.next())
							 {
								 s = s+" <TR>"
											+"<TD>"+myRs3.getInt(1)+"</td>"
													...
													+"<td><form action=\"TrackorderDetail\" method=\"post\"><button onclick=\"orderCookie("+myRs3.getInt(1)+")\"  id=\"btn\">ORDER DETAIL</button></form></TD>"
											+"</TR>";
							 }
						}
					...
					{
						System.out.println("no orderhistory");
						request.setAttribute("error", "<font face=\"Comic sans MS\" color=red>NO ORDER HISTORY</font>");
					}
						mystmt.close();
						myConn.close();
